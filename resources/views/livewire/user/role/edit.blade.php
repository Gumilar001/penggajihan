    ...
    <div class="bg-white shadow-sm rounded-3xl w-full p-[30px]">
        <div class="flex items-center gap-4 mb-7">
            ...
            <div class="flex flex-col">
                ...
                <div class="text-sm text-neutral-60">
                    Halaman ini digunakan untuk mengubah Role
                </div>
            </div>
        </div>
        ...
        <div>
            <div
                ...
                <div class="flex-1">Nama Akses</div>
...
@push('script')
    <script>
        document.addEventListener('livewire:load', function() {
            // get list selected permission id
            var permission_id = @this.permission_id // array

            // set checked permission
            permission_id.forEach(id => {
                $(`[data-id="${id}"]`).each(function(e) {
                    $(this).prop('checked', true)
                })
            });

            // check if select all permission feature
            for (let index = 0; index < permission_id.length; index++) {
                const id = permission_id[index];

                var countChild = $('[data-parent="' + id + '"]').length
                var countChecked = 0
                if (countChild > 0) {
                    ...
                    if (countChild == countChecked) $('[parent-id="' + id + '"]').prop('checked', true)
                }
            }
            ...
            var countTotal = $('.list-permissions-add input[type="checkbox"]').length
            var countTotalChecked = 0;

            if (countTotal > 0) {

                $('.list-permissions-add input[type="checkbox"]').each(function(e) {
                    var checked = $(this).prop('checked');
                    if (checked) countTotalChecked++
                })

                if (countTotal == countTotalChecked) {
                    livewire.emit('setAccess', 'all')
                } else {
                    livewire.emit('setAccess', 'custom')
                }
            }
