    ...
    <div class="bg-white shadow-sm rounded-3xl w-full p-[30px] min-h-[300px] mb-6">
        <div class="flex items-center gap-4 mb-5">
            ...
            <div class="font-bold title-page text-black-80">
                Laporan Penggajihan TNI
            </div>
        </div>
        <div class="flex items-center gap-[10px]">
            <div class="flex-1">
                <div wire:ignore class="self-stretch">
                    <select class="self-stretch select2-tni-bulan" name="state">
        ...
        @if ($generalFilter['bulan_id'] != null)
...
@push('script')
    <script>
        function renderSelectBulanTni() {
            $('.select2-tni-bulan').select2({
                placeholder: "Pilih Bulan Penggajihan TNI",
            });
            $('.select2-tni-bulan').on('change', function(e) {
                let data = $(this).val();
                console.log(data)
                Livewire.emit('setGeneralFilter', 'bulan_id', data);
            });

        }
        renderSelectBulanTni();
    </script>
@endpush
