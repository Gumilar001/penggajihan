<div>
    <div class="bg-white shadow-sm rounded-3xl w-full p-[30px] min-h-[300px]">
        ...
        <div
            ...
            <div class="flex-1 w-full min-[400px]:!w-1/2">
                <div class="relative">
                    <input type="text" placeholder="Cari Nama Personel TNI" wire:model="search"
            ...
            <div class="flex items-center gap-2 sm:flex-wrap w-full min-[400px]:!w-auto">
                <button wire:click="addPenggajihanTni('add')"
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th class="text-center">Nama TNI</th>
                    ...
                    @if (count($data) > 0)
                        @foreach ($data as $key => $item)
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="mt-[5px]">
                                        {{ $item->PersonelTni->pangkatTniId->nama_pangkat }}
                                    </div>
                                </td>
                                <td class="text-center">
                                    <div class="mt-[5px]">
                                ...
                                <td class="text-center">
                                    <div class="flex justify-center gap-1">
                                        ...
                                        <button wire:click="deletePenggajihanTni({{ $item->id }})"
    ...
    <div data-bs-toggle="modal" data-bs-target="#modalPenggajihanTni" class="hidden" id="triggerAddPenggajihanTni">
    </div>
    @include('components.modal.penggajihan.modal-penggajihan-tni')
</div>

@push('script')
    <script>
        $(document).ready(function() {
            window.livewire.on('show-modal-penggajihan-tni', (data, type) => {
                $("#triggerAddPenggajihanTni").click();
                $(".select2-personel-tni").val('').trigger('change')

                renderSelect2()
                $(".select2-personel-tni").val('').trigger('change')
                if (data) {
                    $(".select2-personel-tni").val(data['id_personel']).trigger('change');
                }
            });

            window.livewire.on('close-modal-penggajihan-tni', () => {
                $("#modalPenggajihanTni .btn-cancel").click()
                console.log($("#modalPenggajihanTni .btn-cancel").click())
            });
            $('.select2-personel-tni').on('select2:close', function(e) {
                try {
                    setTimeout(() => {
                        renderSelect2()
                    }, 200);
            ...
            function renderSelect2() {
                $('.select2-personel-tni').select2({
                    dropdownParent: $('#modalPenggajihanTni')
                });

                $('.select2-personel-tni').on('select2:select', function(e) {
                    var data = e.params.data;
                    console.log(data.id)
                    livewire.emit('setPostPersonelTni', 'id_personel', data.id)
                });
                $('.select2-personel-tni').on('select2:select', function(e) {
                    var data = e.params.data;
                    console.log(data.id)
                    livewire.emit('selectPersonelTni', 'id_personel', data.id)
                });
            }
        })
    </script>
