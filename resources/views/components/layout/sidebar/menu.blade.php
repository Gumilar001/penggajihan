@if (Request::segment(1) == str_replace('/', '', $to) || Request::segment(1) == $parent)
    <a href="{{ $to }}" class="menu-sidebar group bg-secondary-10 before:bg-secondary-50" data-type="{{ $menuType }}">
        <div class="[&>svg>path]:fill-secondary-60">
            {{ $icon }}
        </div>
...
@else
    <a href="{{ $to }}" class="menu-sidebar group" data-type="{{ $menuType }}">
        <div class="group-hover:[&>svg>path]:fill-secondary-60">
            {{ $icon }}
        </div>
